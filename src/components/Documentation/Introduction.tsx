  ...
  return (
    <>
      <div id="version" className="md:scroll-m-[180px] scroll-m-28">
            ...
            PackageVersions && PackageVersions.map((item) => {
              return (
                <div key={item.id} className="lg:w-1/5 md:w-full text-center lg:border-b-0 border-b lg:border-e lg:last:border-e-0 last:border-b-0 border-black/10 dark:border-white/20">
                  <Icon icon={item.icon} className="mx-auto w-10 h-10" width={40} height={40} />
        ...
        <div className="mt-5">
          <p className="text-base font-medium text-midnight_text dark:text-white dark:text-opacity-50">SINERTEK Electronics Template построен на Tailwindcss и Next.js.</p>
          <p className="text-base font-medium text-midnight_text dark:text-white dark:text-opacity-50">Этот шаблон готов к использованию и может быть полностью настроен под ваши требования.</p>
          <p className="text-base font-medium text-midnight_text dark:text-white dark:text-opacity-50">Для кастомизации необходимо знание Next.js, React, Tailwind и JSX.</p>
        </div>
